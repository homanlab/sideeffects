...
for (i in 1:length(indices)) {
  ...
  # forest plot
  fpl2 <- gg_forest(fdd = fdd2, 
                   ylab = "Drug",
                   xlab = "Variability ratio (VR)",
                   greater1 = "Greater in placebo",
  ...
  # plot grid
  fplg <- plot_grid(fpl1[[1]],
                    plot_grid(fpl2[[1]], NULL, rel_heights=c(rels3[i], 1),
                              ncol=1),
                    labels=c("A", "B"),
                    label_size=25,
                    scale=c(0.9, 0.9),
                    nrow=1)
  fn <- paste0("../output/figures/", index, "_fig3.pdf")
  cat(paste0("saving figure ", fn, " to disk ..."))
  ggsave(plot=fplg,
         filename=fn,
         width=widths3[i], height=heights3[i])
  cat(" done!", sep="\n")
